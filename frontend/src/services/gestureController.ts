...
const SWIPE_THRESHOLD = 0.01; // Минимальная скорость для swipe (сильно снижено для лучшей отзывчивости)
...
function calculateZoom(
  ...
  // Дополнительный плавный зум при приближении/отдалении зажатой кисти
  ...
  if (avgExtension < 0.3 && currentWrist && previousWrist) {
    ...
    const deltaZ = previousWrist.z - currentWrist.z; // Положительное = приближение, отрицательное = отдаление
    
    if (deltaZ > 0) {
      // Кисть приближается - добавляем дополнительный зум ин
      // Чем больше приближение, тем больше зум
      const distanceZoom = deltaZ * HAND_DISTANCE_ZOOM_SENSITIVITY;
      targetZoom += distanceZoom;
      
      console.log('🔍 Distance zoom IN:', {
        deltaZ,
        distanceZoom,
        avgExtension,
        targetZoom,
      ...
      // Кисть отдаляется - добавляем дополнительный зум аут
      // Чем больше отдаление, тем меньше зум
      const distanceZoom = deltaZ * HAND_DISTANCE_ZOOM_SENSITIVITY; // deltaZ отрицательное, поэтому зум уменьшится
      targetZoom += distanceZoom;
      
      console.log('🔍 Distance zoom OUT:', {
        deltaZ,
        distanceZoom,
        avgExtension,
        targetZoom,
...
function detectSwipe(
  ...
  const MIN_DISTANCE = 0.005; // Минимальное горизонтальное движение (сильно уменьшено для лучшей отзывчивости)
